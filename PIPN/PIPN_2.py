...
class PointNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn2(self.conv2(x)))  #1*128*4231
        x = F.relu(self.bn3(self.conv3(x)))   #1*1024*4231
...
X = torch.rand(1, num_points, Nd)  # Batch size of 1
...
#Loss function
def ComputeCost_SE(X, Y, model, viscosity, cfd_u, cfd_v, cfd_p, pose_BC, pose_sparse, pose_interior, pose_BC_p, pose_sparse_p, pose_interior_p):
    # Ensure gradients are being calculated
    X.requires_grad = True
    Y = model(X)

    # Calculate gradients using autograd
    du_dx_in = grad(Y[0][:, 0], X, create_graph=True, grad_outputs=torch.ones_like(Y[0][:, 0]))[0][..., 0].view(-1)[pose_interior_p]
    du_dy_in = grad(Y[0][:, 0], X, create_graph=True, grad_outputs=torch.ones_like(Y[0][:, 0]))[0][..., 1].view(-1)[pose_interior_p]
    dv_dx_in = grad(Y[0][:, 1], X, create_graph=True, grad_outputs=torch.ones_like(Y[0][:, 1]))[0][..., 0].view(-1)[pose_interior_p]
    dv_dy_in = grad(Y[0][:, 1], X, create_graph=True, grad_outputs=torch.ones_like(Y[0][:, 1]))[0][..., 1].view(-1)[pose_interior_p]
    dp_dx_in = grad(Y[0][:, 2], X, create_graph=True, grad_outputs=torch.ones_like(Y[0][:, 2]))[0][..., 0].view(-1)[pose_interior_p]
    dp_dy_in = grad(Y[0][:, 2], X, create_graph=True, grad_outputs=torch.ones_like(Y[0][:, 2]))[0][..., 1].view(-1)[pose_interior_p]

    d2u_dx2_in = grad(du_dx_in, X, grad_outputs=torch.ones_like(du_dx_in), create_graph=True)[0][..., 0].view(-1)[pose_interior_p]
    d2u_dy2_in = grad(du_dy_in, X, grad_outputs=torch.ones_like(du_dy_in), create_graph=True)[0][..., 1].view(-1)[pose_interior_p]
    d2v_dx2_in = grad(dv_dx_in, X, grad_outputs=torch.ones_like(du_dx_in), create_graph=True)[0][..., 0].view(-1)[pose_interior_p]
    d2v_dy2_in = grad(dv_dy_in, X, grad_outputs=torch.ones_like(dv_dy_in), create_graph=True)[0][..., 1].view(-1)[pose_interior_p]
    ...
    Sparse_cost = torch.mean((u_sparse - sparse_u_truth) ** 2 + (v_sparse - sparse_v_truth) ** 2 + (p_sparse - sparse_p_truth) ** 2)
    Sparse_cost = Sparse_cost.detach().numpy()

    return (100.0 * PDE_cost + 100.0 * Sparse_cost + BC_cost)


#训练
def build_model_PIPN_PorousMedia(model, X_train, BC_list, sparse_list, interior_list, num_points, data, Np, Nb, LR, J_Loss):
    min_loss = 1000
    ...
    for epoch in range(Np):
        ...
        for sb in range(int(data / Nb)):
            ...
            cost = ComputeCost_SE(X_train_mini, outputs, model, viscosity, cfd_u, cfd_v, cfd_p, pose_BC, pose_sparse, pose_interior, pose_BC_p, pose_sparse_p, pose_interior_p)
            temp_cost += cost.item()
        ...
        if temp_cost < min_loss:
            u_final = np.power(outputs[:, :, 0].detach().numpy(), 1.0)
            v_final = np.power(outputs[:, :, 1].detach().numpy(), 1.0)
            p_final = np.power(outputs[:, :, 2].detach().numpy(), 1.0)
        ...
        # Plotting and error analysis (adapt as needed)
    for index in range(data):
        ...
        plotCost(cost_all, 'cost', 'Cost function')
...
build_model_PIPN_PorousMedia( model, X_train, BC_list, sparse_list, interior_list, num_points, data, Np, Nb, LR, J_Loss)
