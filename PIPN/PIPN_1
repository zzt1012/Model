...
class PointNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.tanh(self.bn2(self.conv2(x)))  # 1*128*4231
        x = torch.tanh(self.bn3(self.conv3(x)))  # 1*1024*4231
        ...
        x = torch.tanh(self.bn6(self.conv6(x)))
...
def ComputeCost_SE(device, X, model, viscosity, cfd_u, cfd_v, cfd_p, pose_BC, pose_sparse, pose_interior, pose_BC_p,
    ...
    return (1 * PDE_cost + 1.0 * Sparse_cost + 1000 * BC_cost)
...
def build_model_PIPN_PorousMedia(device, model, X_train, BC_list, sparse_list, interior_list, num_points, data, Np, Nb, LR,
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=Nb)
    ...
    for epoch in range(Np):
        ...
        for sb in range(int(data / Nb)):
            ...
            optimizer.step()
            #print('{} scheduler'.format(scheduler.get_last_lr()))
